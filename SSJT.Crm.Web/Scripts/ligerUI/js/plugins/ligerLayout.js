    ...
    $.fn.ligerLayout = function (p) {
        return this.each(function () {
                ...
                onResize: function () {
                    ...
                    if (p.height) {
                        if (typeof (p.height) == 'string' && p.height.indexOf('%') > 0) {
                            if (g.layout.parent()[0].tagName.toLowerCase() == "body") {
                                var windowHeight = $(window).height();
                                windowHeight -= parseInt(g.layout.parent().css('paddingTop'));
                                windowHeight -= parseInt(g.layout.parent().css('paddingBottom'));
                                g.layout.height(windowHeight * parseFloat(p.height) * 0.01);
                            }
                            else
                                g.layout.height((g.layout.parent().height() * parseFloat(p.height) * 0.01));
                        }
                        else {
                            g.layout.height(p.height);
                        }
                    }
            ...
            if ($("> div[position=center]", g.layout).length > 0) {
                ...
                if (!g.center.content.hasClass("l-layout-content"))
            ...
            if ($("> div[position=right]", g.layout).length > 0) {
                ...
                g.right.content = $("> div[position=left]", g.right);
