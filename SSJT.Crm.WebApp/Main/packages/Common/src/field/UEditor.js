Ext.define('Common.field.UEditor', {
    ...
    afterRender(){
        ...
        if(!RM.isDefined(bundleId)) {
            const path = Ext.getResourcePath('UEditor/','shared','Common'),
                isDev = Ext.manifest.env === 'development',
                min = isDev?'':'.min',
                version = Ext.manifest.version;
                window.UEDITOR_HOME_URL = path;
                window.NEDITOR_UPLOAD = isDev ? 'http://localhost:55441/Doc/ueditor/UEditorHandler.ashx' : '../Doc/ueditor/UEditorHandler.ashx';
            const mask = Ext.widget('loadmask',{
                message:'正在加载编译器',
                renderTo:me.bodyElement
            });
            RM.load([
                `${path}neditor.config.js?v=${version}`,
                `${path}neditor.all${min}.js?v=${version}`
            ],bundleId,{
                success(){
                    mask.destroy();
                },
